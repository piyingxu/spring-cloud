...
public class ZooKeeperWatcher implements Watcher {
    ...
    /*
        CountDownLatch是JAVA提供在java.util.concurrent包下的一个辅助类，可以把它看成是一个计数器，其内部维护着一个count计数，只不过对这个计数器的操作都是原子操作，同时只能有一个线程去操作这个计数器，
        CountDownLatch通过构造函数传入一个初始计数值，调用者可以通过调用CounDownLatch对象的cutDown()方法，来使计数减1；如果调用对象上的await()方法，
    ...
    */
    ...
    /** 进入标识 */
    private static final String LOG_PREFIX_OF_MAIN = "【MyWatch】";
    /** zk变量 */
    private ZooKeeper zk = null;

    public ZooKeeperWatcher() {
        createConnection(CONNECTION_ADDR, SESSION_TIMEOUT);
    }
    ...
    public void process(WatchedEvent event) {
        ...
        if (Event.KeeperState.SyncConnected == keeperState) {
            ...
            //更新节点
                ...
                log.info(logPrefix + "数据内容: " + this.readData(path, true));
            }
            //更新子节点
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (zkWatch.createPath(PARENT_PATH, System.currentTimeMillis() + "")) {
            ...
            // 读取数据
            log.info("---------------------- read parent ----------------------------");
            //zkWatch.readData(PARENT_PATH, true);

            // 读取子节点
            log.info("---------------------- read children path ----------------------------");
